...
function analytical_approach(param)
	...
	mu = param.θ_noncoding/(4*param.N)
...
function sim_table(alphas,bgs_values,p_size,n_size,l)
	...
	adap = MKtest.parameters(N=p_size,n=n_size,gam_neg=-457,Lf=2*10^5, B=0.999,gL=10,gH=500,al_tot=0.4,al_low=0.2,B_bins=[0.9,0.999])
