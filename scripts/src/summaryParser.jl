...
function plotMapSimulations(case,replicas,output,title,simulations="/home/jmurga/mkt/202004/rawData/simulations/")
	...
	p = R"""if(length(unique(dfAlphas[['w']])) > 1){
	    dfAlphas[['w']] = factor(dfAlphas[['w']], labels = c('alpha[w]:0.1','alpha[w]:0.2','alpha[w]:0.3'))
		}else{
		    dfAlphas[['w']] = factor(dfAlphas[['w']], labels = c('alpha[w]:0.1'))
		}	
		if(length(unique(dfAlphas[['b']])) > 1){
		    dfAlphas[['b']] = factor(dfAlphas[['b']], labels = c('B:0.2','B:0.4','B:0.8','B:0.999'))
		}else{
		    dfAlphas[['b']] = factor(dfAlphas[['b']], labels = c('B:0.999'))
		}

		d = melt(dfAlphas, id.vars=c('method','w','b','analysis'),measure.vars=c('aw','as','a'))

		p   = ggplot(d) + geom_boxplot(aes(x=variable,y=value,fill=method),alpha=0.8) + facet_grid(b~w,labeller=label_parsed) + scale_fill_manual(values= c('#ab2710', '#e2bd9a', '#30504f'),name="Method") + colourSanMiguel() + theme_bw() + labs(title=$title) + xlab("Parameter") + ylab("Value");
