...
################
reduceSfs = function (sfsTemp,bins){

	bn = bins*2

	freq  = seq(0:(nrow(sfsTemp)-1))/nrow(sfsTemp)
	freqRanges = cut(freq, breaks = seq(0,1,(1/(bn-1))),labels = seq(1/(bn-1),1,(1/(bn-1))))

	tmp = cbind(sfsTemp,freqRanges)
	out = list()
	
	vIter =  levels(freqRanges)
	for(i in vIter){
		out[[paste(i)]] = tmp[freqRanges %in% i][,2:(ncol(tmp)-1)]  %>% summarise_all(sum)

		
	}
	
	reducedSfs = cbind(seq(1/(bn-1),1,(1/(bn-1))),rbindlist(out))
	names(reducedSfs)[1] = 'f'
	return(reducedSfs)
}

sfsToPolyDFE = function(path,bins,output="/home/jmurga/mkt/202004/rawData/dofe/noDemog/")
{
	sfs = fread(paste0(path,"/sfs.tsv"),sep='\t')
	div = fread(paste0(path,"/div.tsv"),sep='\t')
	analysis = tail(unlist(strsplit(path,"/"))[-1],1)
	if (((bins*2)-1) != nrow(sfs)){

		sfs = reduceSfs(sfs,bins)
		header = t(data.table(c(1,1,(bins*2)))) %>% as.data.table
		fName = paste0(output,analysis,"/",analysis,"_polydfe_",as.character(bins),".tsv")
	} else{
		header = t(data.table(c(1,1,nrow(sfs)))) %>% as.data.table
		fName = paste0(output,tail(unlist(strsplit(path,"/"))[-1],1),"_polydfe.tsv")
	}

	neutral = c(sfs[,3][[1]],10^8*0.25,div[[1]],10^8*0.25)
	selected = c(sfs[,2][[1]],10^8*0.75,div[[1]],10^8*0.75)
	df = data.table(neutral,selected)	 %>% t %>% as.data.table
	format(df, drop0trailing = TRUE, scientific = FALSE)     
	
	fwrite(header,fName,sep=' ',col.names=F)
	fwrite(df,fName,sep=' ',col.names=F,append=T,scipen=50)
}

plotEstimatedSFS = function(sfsFile,divFile,dfeFile,output){
	
	sfs = parseSFSData(sfsFile)
	est_div = parseOutput(divFile)[[1]]
	sfs_div = est_div$expec[[1]]


	est_dfe = parseOutput(divFile)[[1]]
	sfs_dfe = est_dfe$expec[[1]]

	sfs_div = sfs_div * (sfs[, "length_sfs"])
	sfs_dfe = sfs_dfe * sfs[, "length_sfs"]

	n = max(grep("E[P", colnames(sfs_div), fixed = TRUE))
	m = min(sfs[, 1:n], sfs_div[, 1:n])
	M = max(sfs[, 1:n], sfs_div[, 1:n])

	# svg(output)
	plot(1, 1, type = "n", ylim = c(m, M), xlim = c(1, n), xlab = "i", ylab = "SFS", log = "y")
	xleft = seq(from = 1, to = n, by = 2)
	rect(xleft = xleft - 0.5, ybottom = m * 0.001, xright = xleft + 0.5, ytop = M * 100, col = gray(0.9), border = NA)
	box()

	for (i in 1:2)
	{
		points(1:n - 0.15, sfs_div[i, 1:n], col = "red", pch = 19 + i)
		points(1:n + 0.15, sfs_dfe[i, 1:n], col = "blue", pch = 19 + i)
	...
	legend = c("obs", "expec with div", "neutral", "selected"))
	dev.off()
}

plotDiscreteDfe = function(est){

	dfe = t(sapply(est, getDiscretizedDFE, c(-100, -10, -1, 0, 5,10)))
	par(mfrow = c(1, 2))
	...
	legend("topright", legend = rownames(dfe), fill = col, bty = "n", ncol = 3)
}
